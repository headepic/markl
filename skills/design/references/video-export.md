...
```bash
NODE_PATH=$(npm root -g) node /path/to/design/scripts/render-video.js <html文件>
```
...
```bash
bash /path/to/design/scripts/convert-formats.sh <input.mp4> [gif_width] [--minterpolate]
```
