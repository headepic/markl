...
2. 输入 "set up ai feeds" 或执行 `/ai-feeds`（旧的 `/ai` 触发方式依然可用）
...
Skill 本地名为 `ai-feeds`，从上游仓库 `zarazhangrui/follow-builders` 安装到本地 `ai-feeds` 目录。
...
- 如果你使用 Telegram/邮件推送，相关 key 仅存储在本地 `~/.ai-feeds/.env`
