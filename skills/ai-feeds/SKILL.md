---
name: ai-feeds
description: AI Feeds — monitors top AI builders on X and YouTube podcasts, remixes their content into digestible summaries. Use when the user wants AI industry insights, builder updates, or invokes /ai or /ai-feeds. No API keys or dependencies required — all content is fetched from a central feed.
---

# AI Feeds — Follow Builders, Not Influencers
...
Check if `~/.ai-feeds/config.json` exists and has `onboardingComplete: true`.
...
```bash
cat > ~/.ai-feeds/config.json << 'CFGEOF'
{
...
openclaw cron add \
  ...
  --session isolated \
  --message "Run the ai-feeds skill: execute prepare-digest.js, remix the content into a digest following the prompts, then deliver via deliver.js" \
  --announce \
...
Read `~/.ai-feeds/config.json` for user preferences.
...
### Step 7: Offer to ingest into llm-wiki (user confirmation required)

After delivering the digest, check whether ingest into the llm-wiki `ai-feeds`
wiki is possible and, if so, **ask the user to confirm before ingesting**.
Never ingest automatically. This is a hard rule.

**① Check preconditions silently** (do not report these steps to the user):

```bash
# Does the ai-feeds wiki exist?
ls ~/Documents/llm-wiki/ai-feeds/SCHEMA.md 2>/dev/null && echo "WIKI_EXISTS" || echo "NO_WIKI"

# Has today's digest already been ingested?
ls ~/Documents/llm-wiki/ai-feeds/raw/digests/$(date +%Y-%m-%d).md 2>/dev/null && echo "ALREADY_INGESTED" || echo "NOT_YET"
```

- If the wiki does **not** exist → skip silently, do not ask the user to create it.
- If today's digest is **already** ingested → skip silently, do not re-ingest.

**② If the wiki exists and today is not yet ingested**, ask the user:

> "要不要把今天的 digest ingest 到 llm-wiki 的 ai-feeds？"
> ("Would you like me to ingest today's digest into the ai-feeds wiki?")

**③ Wait for the user's answer.** Do NOT proceed until the user explicitly
confirms (e.g. "yes", "好", "ingest", "go ahead"). If the user declines or
ignores the question, stop — the digest was already delivered, your run is
done. Do not nag.

**④ Only on explicit confirmation**, invoke the llm-wiki skill to do the ingest:

```
/llm-wiki @ai-feeds ingest <today's date digest>
```

Pass the full digest text (or its file path) as the source. The llm-wiki skill
will handle the rest:
1. Save the digest as `raw/digests/YYYY-MM-DD.md`
2. Create or update entity pages (people, products, companies)
3. Create or update concept pages (topics, techniques)
4. Update index.md and log.md

**Why this is gated**: wiki ingest creates durable, compounding state and can
touch many existing pages. The user should own that decision, not the cron.
For cron-driven runs where there is no interactive user, the cron message
should be structured so the ingest step is opt-in (e.g. the user invokes a
separate command later, or the cron message explicitly asks and waits on a
reply channel). Never bypass the confirmation because "no one is watching".
...
open an issue at the upstream repo https://github.com/zarazhangrui/follow-builders."
...
file to `~/.ai-feeds/prompts/` and edit it there. This way their
customization persists and won't be overwritten by central updates.

```bash
mkdir -p ~/.ai-feeds/prompts
cp ${CLAUDE_SKILL_DIR}/prompts/<filename>.md ~/.ai-feeds/prompts/<filename>.md
```

Then edit `~/.ai-feeds/prompts/<filename>.md` with the user's requested changes.
...
- "Reset to default" → Delete the file from `~/.ai-feeds/prompts/`
