...
2. Say "set up ai feeds" or invoke `/ai-feeds` (the older `/ai` trigger also works)
...
The skill is locally named `ai-feeds`. It is installed from the upstream
repo `zarazhangrui/follow-builders` and placed at `ai-feeds`.

### OpenClaw
```bash
# From ClawhHub (coming soon)
clawhub install ai-feeds

# Or manually
git clone https://github.com/zarazhangrui/follow-builders.git ~/skills/ai-feeds
cd ~/skills/ai-feeds/scripts && npm install
```

### Claude Code
```bash
git clone https://github.com/zarazhangrui/follow-builders.git ~/.claude/skills/ai-feeds
cd ~/.claude/skills/ai-feeds/scripts && npm install
```
...
- If you use Telegram/email delivery, those keys are stored locally in `~/.ai-feeds/.env`
